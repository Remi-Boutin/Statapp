...
for k in list_names:
    os.chdir(dossier+'twitter-users/'+k)
    f = glob.glob('*.json')
...
for  h in list_names:
    os.chdir(dossier+'twitter-users/'+h)
    f = glob.glob('*.json')
...
with open(dossier+'compte_id.json') as outf:
...
with open(dossier+'compte_id_edit.json','w') as outf:
